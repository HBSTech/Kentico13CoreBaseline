...
namespace Generic.Models
{
    public class SiteMapOptions : ICacheKey
    {
        ...
        {
            return (!string.IsNullOrWhiteSpace(CacheItemName) ? $"{Path}|{(ClassNames != null ? string.Join(",",ClassNames) : "")}|{CombineWithDefaultCulture}|{CultureCode}|{MaxRelativeLevel}|{SelectOnlyPublished}|{WhereCondition}|{CheckDocumentPermissions}" : CacheItemName);
        }
    }
}